...
class RandomReverseComplement(nn.Module):
    """Stochastically reverse complement a one hot encoded DNA sequence."""

    def __init__(self, **kwargs):
        super(RandomReverseComplement, self).__init__()
...
class RestoreReverseOrientation(nn.Module):
    """Reverse predictions if the inputs were reverse complemented."""

    def __init__(self, **kwargs):
        super(RestoreReverseOrientation, self).__init__()
...
class RandomSequenceShift(nn.Module):
    """Stochastically shift a one hot encoded DNA sequence."""

    def __init__(self, shift_max=0, pad="uniform", **kwargs):
        super(RandomSequenceShift, self).__init__()
    ...
    def forward(self, seq_1hot):
        if self.training:
            ...
            if torch.not_equal(shift, 0):
                sseq_1hot = apply_sequence_shift(seq_1hot, shift)
            else:
                sseq_1hot = seq_1hot
...
def apply_sequence_shift(seq, shift, pad_value=0.25):
...
def format_run_metadata(argv: List[str], args: Namespace=None, **kwargs) -> str:
    s = list()
    s.append("")
...
def ensure_output_directory(in_dir):
...
def initialize_random_seed(seed: int, force_deterministic: bool=False):
    """
    set seed
    """
