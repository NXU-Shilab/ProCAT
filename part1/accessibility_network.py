...
from channel_attention import AdaptiveChannelAttention
...
class ResidualConvBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel: int, kernel_size) -> None:
        ...
        self.eca = AdaptiveChannelAttention(out_channel)
...
class AccessibilityPredictor(nn.Module):
    """
    ...
    def __init__(self, n_cells: int, batch_ids: Optional[Iterable[int]]=None, use_reg_cell=False, hidden_size=32, seq_len: int=1344):
        ...
        # 2
        self.conv_towers = []
        self.conv_towers.append(ResidualConvBlock(288, 64, 5))  # output: (batch_size, 64, 224)
        current_len = current_len // 2
        self.conv_towers.append(ResidualConvBlock(64, 128, 5))  # output: (batch_size, 128, 112)
        current_len = current_len // 2
        self.conv_towers.append(ResidualConvBlock(128, 256, 5)) # output: (batch_size, 256, 56)
        current_len = current_len // 2
        self.conv_towers.append(ResidualConvBlock(256, 512, 5)) # output: (batch_size, 512, 28)
        current_len = current_len // 2
